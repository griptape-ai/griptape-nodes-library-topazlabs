...
from constants import DENOISE_DEFAULTS, DENOISE_MODELS, PARAMETER_RANGES
        ...
        try:
            ...
            if image_artifact is None:
                ...
                if input_param and hasattr(input_param, "connections"):
                    self._update_status(
                        "Image parameter exists but no value received - checking connections...", show=True
                    )
                else:
            ...
            output_artifact = self._save_image_output(processed_image_data, filename_hint="denoised_image")
    ...
    def validate_before_node_run(self) -> list[Exception] | None:
        ...
        if minor_deblur is not None:
            ...
            if not (minor_deblur_range[0] <= minor_deblur <= minor_deblur_range[1]):
                errors.append(
                    ValueError(f"Minor deblur must be between {minor_deblur_range[0]} and {minor_deblur_range[1]}")
                )
        ...
        if original_detail is not None:
            ...
            if not (original_detail_range[0] <= original_detail <= original_detail_range[1]):
                errors.append(
                    ValueError(
                        f"Original detail must be between {original_detail_range[0]} and {original_detail_range[1]}"
                    )
                )
