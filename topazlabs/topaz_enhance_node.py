...
from constants import ENHANCE_DEFAULTS, ENHANCE_MODELS, PARAMETER_RANGES
...
class TopazEnhanceNode(BaseTopazNode):
    ...
    async def _process_async(self) -> None:
        ...
        try:
            ...
            api_params = {"model": model, "output_format": output_format}
            ...
            with self._get_topaz_client() as client:
                processed_image_data = client.enhance(image_data=image_data, **api_params)

            # Save output image
            self._update_status("Saving enhanced image...", show=True)
            output_artifact = self._save_image_output(processed_image_data, filename_hint="enhanced_image")
